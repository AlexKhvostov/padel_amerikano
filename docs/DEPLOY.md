...
`sql/migrations/008_tournament_activity.sql`, затем
`sql/migrations/009_score_archive_email.sql`.

Если миграция `002` уже применялась ранее, выполните последовательно `003` … `009`.
Если `008` уже применялась — выполните только `009_score_archive_email.sql`.
...
Затем выполните `009_score_archive_email.sql`: архив турниров, `admin_email`
и поле для напоминания кода. Для уже существующих компаний напоминание
заработает после первого успешного входа администратора (и указания email в настройках).
